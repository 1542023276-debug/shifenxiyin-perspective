...
> **分层加载（progressive disclosure）**：这是 **v1.10.0 的结构改造**。以前所有内容都塞在 SKILL.md 里（85KB），每次激活都被整体读入上下文、token 开销大。现在 SKILL.md 只留「每次都要用」的部分（8KB），细节拆到 `references/` 按需读取——**入口文件保留触发逻辑与路由表当索引**，需要哪个模型/哪个场景再读哪一份。README 与 SKILL.md 都给出了路由表。

> **说明**：调研文档编号（00/03/05）对应调研阶段的工作文档编号；**编号 01 现由纲领性文件《吸引子方法论》占用**。原 01（心智模型草稿）和 02/04（中间过程笔记）为内部调研产物，已整合进分包，不再单独分发。v1.10.0 起所有细节文档统一收在 `references/` 下并按主题重新编号。
